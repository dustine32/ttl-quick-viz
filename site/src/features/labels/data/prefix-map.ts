// CURIE prefix → IRI prefix map.
//
// Snapshotted from old-noctua-visual-pathway-editor/src/@noctua.curie/data/go-context.ts.
// We use this only for the labels feature (CURIE ⇄ IRI conversion at the GOlr
// boundary). Display-time prefixing in features/view-config/prefixes.ts keeps
// its own minimal DEFAULT_PREFIXES; that's display config, this is protocol.

export type PrefixMap = Record<string, string>;

export const GO_CONTEXT: PrefixMap = {
...
// Sorted entries [prefix, namespace] descending by namespace length —
// guarantees the longest-match wins (e.g. "GO" beats "obo" for
// http://purl.obolibrary.org/obo/GO_*).
export const SORTED_PREFIX_ENTRIES: ReadonlyArray<readonly [string, string]> =
  Object.entries(GO_CONTEXT).sort((a, b) => b[1].length - a[1].length);
