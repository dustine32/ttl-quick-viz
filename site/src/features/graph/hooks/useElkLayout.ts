...
export function useElkLayout(
  ...
  heightFor?: (nodeId: string) => number,
): UseElkLayoutResult {
  ...
  useEffect(() => {
    ...
    const heightOf = (id: string): number => heightFor?.(id) ?? NODE_HEIGHT;

    const elkInput = {
      id: 'root',
      layoutOptions: { ...getElkOptions(algo), ...(layoutOptionsOverride ?? {}) },
  ...
  }, [graph, algo, nonce, layoutOptionsOverride, widthFor, heightFor]);
