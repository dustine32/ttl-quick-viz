import { graphSlice, setRenderer, setSelectedGraphId } from '@/features/graph/slices/graphSlice';

describe('graphSlice', () => {
  it('starts with empty selectedGraphId and renderer = "xyflow"', () => {
    const state = graphSlice.reducer(undefined, { type: '@@INIT' });
    expect(state.selectedGraphId).toBe('');
    expect(state.renderer).toBe('xyflow');
  });

  it('setSelectedGraphId updates the id', () => {
    const state = graphSlice.reducer(
      { selectedGraphId: '', renderer: 'xyflow' },
